...
t2 <- list.files(paste0(model,"/assigned_rates/raw_rates"),pattern="sim_rates_info_rep",full.names=T)
for (i in 1:length(t1)) {
  f1 <- t1[i] #sim_rates
  f2 <- t2[i] #sim_rates_info
  
  name_check1 <- substr(f1,50,120)
  name_check2 <- substr(f2,55,120)
  outfile <- paste0(model,"/assigned_rates/processed_rates/sim_rates_combined_rep",name_check1)
